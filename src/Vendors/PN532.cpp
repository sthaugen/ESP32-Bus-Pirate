...
int PN532::clone(bool checkSak) {
    if (!nfc.startPassiveTargetIDDetection()) return TAG_NOT_PRESENT;
    if (!nfc.readDetectedPassiveTargetID()) return FAILURE;
    if (checkSak && nfc.targetUid.sak != uid.sak) return TAG_NOT_MATCH;
...
int PN532::write(int cardBaudRate) {
    if (cardBaudRate == PN532_MIFARE_ISO14443A) {
        ...
        memcpy(uid.uidByte, idm, 8); uid.size = 8;
    }
...
int PN532::authenticate_mifare_classic(byte block) {
    uint8_t successA = 0;
    uint8_t successB = 0;
    std::vector<String> mifareKeys; // TODO: handle this
    ...
    if (!successA) {
        ...
        for (const auto &mifKey : mifareKeys) {
